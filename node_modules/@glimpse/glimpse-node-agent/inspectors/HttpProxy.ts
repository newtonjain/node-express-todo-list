...
import { IErrorReportingService, createHttpServerError } from '@glimpse/glimpse-node-common';
...
export class HttpProxy extends ProxyBase {
    ...
    private raiseRequestEndEvent(req: http.IncomingMessage, res: http.ServerResponse, content: Array<any>, size: number, requestStartTime: string): void {
        for (const inspector of this.inspectors) {
    ...
    private setupServerProxy(agent: IAgent, httpModule): void {
        ...
        httpModule.createServer = function createServer(options, cb, ...args) {
            function internalCallback(req: http.IncomingMessage, res: http.ServerResponse, ...rest) {
                const requestStartTime = self.getCurrentTimeStamp();
