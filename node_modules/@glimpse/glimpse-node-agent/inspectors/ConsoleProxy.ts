...
interface ProxyMethod {
    (originalThis: Object, originalArgs: any, realMethod: any, stackTopFunction): any;
};
...
export class ConsoleProxy extends ProxyBase {
    ...
    public init(agent: IAgent, consoleModule: any): any {
        ...
        this.addProxy('trace', 'warn', this.trace, consoleModule);  // let "trace" calls go through our override, and then through "warn"
    ...
    private trace(originalThis, originalArgs, realMethod, stackTopFunction) {
        // logic here comes from trace implementation in node: https://github.com/nodejs/node/blob/master/lib/console.js#L82_L90.
        // we do some special stuff to remove glimpse frames from the stack, and to also enable string formatting logic in the client.
        const err = new Error();
        err.name = 'Trace';
        const newArgs = Array.prototype.slice.call(originalArgs, 1);
        err.message = originalArgs[0];
        (<any>Error).captureStackTrace(err, stackTopFunction);
        newArgs.unshift(err.stack);
        return this.proxyHelper(originalThis, newArgs, realMethod, LogLevel.Debug, stackTopFunction);
    }
    ...
    private addProxy(methodToProxy: string, methodToCall: string, proxyFunction: ProxyMethod, consoleModule) {
        ...
        if (consoleModule[methodToProxy]) {
            ...
            function p1() {
                proxyFunction.call(self, this, arguments, self._realConsoleMethods[methodToCall], p1);
            };
        ...
        if (consoleModule.Console.prototype[methodToProxy]) {
            ...
            function p2() {
                proxyFunction.call(self, this, arguments, self._realConsolePrototypeMethods[methodToCall], p2);
            };
