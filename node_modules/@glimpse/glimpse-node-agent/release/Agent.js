...
var glimpse_node_common_2 = require('@glimpse/glimpse-node-common');
var glimpse_node_common_3 = require('@glimpse/glimpse-node-common');
var glimpse_node_common_4 = require('@glimpse/glimpse-node-common');
...
var Agent = (function () {
    function Agent() {
        ...
        var commandLineArgs = glimpse_node_common_3.ConfigSettings.filterCommandLineArgs('--GLIMPSE_');
        var localSettingsFile = glimpse_node_common_3.ConfigSettings.findFile(__dirname, 'glimpse.config.json');
        var defaultSettings = path.join(__dirname, 'glimpse.agent.default.config.json');
        var configSettings = new glimpse_node_common_3.ConfigSettings(commandLineArgs, 'GLIMPSE_', localSettingsFile, defaultSettings);
        var telemetryConfig = glimpse_node_common_4.getTelemetryConfig(configSettings);
        this._telemetryService = new glimpse_node_common_4.TelemetryService(glimpse_node_common_4.GlimpseComponentType.NODE_AGENT, telemetryConfig);
        this._errorReportingService = new glimpse_node_common_1.CompositeErrorReportingService([new glimpse_node_common_1.LoggingErrorReportingService(), new glimpse_node_common_1.TelemetryErrorReportingService(this._telemetryService)]);
    ...
    Agent.prototype.init = function (options) {
        ...
        if (options.server) {
            var packageHelper = glimpse_node_common_1.PackageHelper.instance;
    ...
    Agent.prototype.sendAgentInitEvent = function () {
        var props = {};
        var appData = glimpse_node_common_4.getTelemetryAppInstanceData();
        ...
        var measurements = {};
        this._telemetryService.sendEvent(glimpse_node_common_4.TelemetryEvents.NODE_AGENT_INIT, props, measurements);
    };
        ...
        if (this.providers.configSettings.getBoolean('agent.validate-packages.enabled', true)) {
            ...
            if (packageNames.length > 0) {
                packageNames.forEach(function (packageName) {
                    _this.providers.errorReportingService.reportError(glimpse_node_common_2.createPackageRequiredBeforeInitError(packageName));
                });
            }
        }
    };
