...
var ExpressInspectorMiddleware = (function () {
    ...
    ExpressInspectorMiddleware.prototype.init = function (broker, contextManager, dateTime) {
        ...
        Tracing_1.default.onAlways(ExpressEvents_1.EVENT_EXPRESS_INVOKE_PRE_ROUTE_METHOD, function (event) {
            _this.onMiddlewareMethodInvoked(event.data.originalThis, event.data.originalArgs, event.data.methodName);
        });
    ...
    ExpressInspectorMiddleware.prototype.onMiddlewareMethodInvoked = function (originalThis, originalArgs, method) {
        if (originalThis.glimpse && originalThis.glimpse.ignore) {
            return;
        }
        if (originalArgs && originalArgs.length >= 1) {
            this.wrapNestedMiddleware(originalArgs, [originalThis.path], method !== 'all' ? method.toUpperCase() : undefined);
        }
    };
    ...
    ExpressInspectorMiddleware.prototype.wrapNestedMiddleware = function (array, paths, method) {
        for (var i = 0; i < array.length; i++) {
            ...
            if (typeof arg === 'function') {
                ...
                array[i] = MiddlewareWrapper_1.MiddlewareWrapper.wrap(this.broker, this.contextManager, this.dateTime, paths, method, arg);
            }
