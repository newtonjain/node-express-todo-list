        ...
        if (expectedContextID) {
            ...
            if (currentContextID !== expectedContextID) {
                if (this.errorReportingService) {
                    this.errorReportingService.reportError(glimpse_node_common_1.createUnexpectedContextError(), location, expectedContextID, currentContextID);
                }
            }
        }
        else if (!currentContext) {
            if (this.errorReportingService) {
                this.errorReportingService.reportError(glimpse_node_common_1.createNoContextError(), location);
            }
        }
    };
